-- redeem_admin_recovery_code内でraise exceptionすると、その呼び出し全体(直前に行った
-- recovery_code_attemptsへのINSERTを含む)がロールバックされてしまい、試行回数の記録が
-- 残らずレート制限が機能しないというバグを修正する。
-- raise exceptionをやめ、成功/失敗をレコード(success, message, ...)として返す設計に変更し、
-- 想定される失敗パス(コード誤り・試行回数超過)では例外を発生させないようにする。
...
returns table(success boolean, message text, team_id uuid, team_name text)
