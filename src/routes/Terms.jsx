...
export function Terms() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Section title="第2条(利用開始・共有URL・匿名アカウント等)">
        <ol className="list-decimal pl-5 flex flex-col gap-1">
          <li>本サービスは、チームの作成、または既存チームの共有URL(チームごとに発行される、推測困難なランダムな文字列を含むURL)を用いたアクセスにより利用を開始します。</li>
          <li>本サービスでは、メールアドレスおよびパスワードによる通常の会員登録を行わず、端末等に対して自動的に発行される匿名アカウント(Supabase Authによる匿名認証)を利用して利用者を識別します。</li>
          <li>
            チームの共有URLを知っている者は、本サービス上で特段の資格確認を経ることなく、当該チームの選手情報・試合記録・スタッツ等の閲覧および編集を行うことができます。利用者は、共有URLを自己の責任で適切に管理し、チームへのアクセスを許可する意図のない第三者に開示しないものとします。共有URLの管理不十分により生じた損害について、運営者は責任を負いません。
          </li>
          <li>
            利用者は、チーム設定画面から共有URLを再発行できます。再発行時点で、それ以前に発行された共有URLは無効となります(ただし、既にチームに参加済みの端末による利用は継続されます)。
          </li>
          <li>
            端末の変更、アプリの削除、ブラウザデータの消去その他の事情により、匿名アカウントとの紐付けが失われた場合、当該端末固有の利用状態を復元することはできません。ただし、共有URLを保管している場合は、再度そのURLからアクセスすることで、チーム内の選手情報・試合記録等のデータを引き続き閲覧・編集いただけます。
          </li>
        </ol>
      </Section>
      ...
      <Section title="第3条(禁止事項)">
        ...
        <ol className="list-decimal pl-5 flex flex-col gap-1">
          ...
          <li>不正アクセス、他人の匿名識別子・共有URL等の不正利用その他不正な手段により本サービスを利用する行為</li>
      ...
      <Section title="第5条(登録データの取扱い)">
        <ol className="list-decimal pl-5 flex flex-col gap-1">
          ...
          <li>登録データは、共有URLを用いて同一チームへアクセスした端末間で共有され、共有URLを知る全ての者が閲覧および編集できます。</li>
